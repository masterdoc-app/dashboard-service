...
Env: `PORT` (default 8092), `CATALOG_BASE_URL`, `MAINTENANCE_SERVICE_BASE_URL` (default `http://127.0.0.1:8098`), `FEATURE_SERVICE_BASE_URL` (default `http://127.0.0.1:8082` — target-user feature lookup for assignee eligibility), `AI_MESSAGE_SERVICE_BASE_URL` (default `http://127.0.0.1:8101`), `AI_MESSAGE_INTERNAL_TOKEN`, `BOARD_HORIZON_WEEKS` (default 4).
